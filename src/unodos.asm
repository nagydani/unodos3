...
; Cesar Hernandez Bano, Jeff Braine, Simon Brattel, Miguel Guerreiro,
; Mark Incley, Miguel Angel Rodriguez Jodar, LaesQ, Olof Lagerkvist,
; ub880d, George Velesoft, Matt Westcott

	zoWarnFlow = false;		// prevent pseudo op-codes triggering warnings.

output_bin "../unodos.rom",0,$2000
output_tap "unodos.tap","UNODOS.ROM","",0,$2000,0,$6000
output_bin "unodos-0.sys",$2000,lower_end-$2000
output_bin "unodos-1.sys",$3000,upper_end-$3000
...
L0032:
	defm "/dos/";+ avoids clash with keyword in tokenizers
...
; make screen black;
	...
	call logo;				// print the UnoDOS logo
...
L0169:
	...
	ld (chars), hl;
	ld hl, L1B37;			// address copyright message
	...
	ld ($2d46), a;
	ld ($2d4a), a;
	call chloe_test;		// test for 280SE
...
L1B37:
	...
	defb " 2017 Source Solutions, Inc.", $0d, 0
...
chloe_test:
	ld bc, $80f4;			// LD B, %10000000; LD C, MMU
	ld de, $0001;			// LD D, 0; LD E, 1
	ld hl, $ffff;			// ramptop
	out (c), d;				// page out sideways RAM
	ld (hl), d;				// zero it
	out (c), b;				// page in top 8K of sideways RAM
	ld (hl), e;				// make it 1
	out (c), d;				// page out sideways RAM
	ld a, (hl);				// get value in A
	and a;					// zero? Will be 1 if there is no sideways RAM.
	ld a, ($2d4a);			// restore value of A
	jp nz, L041E;			// return if not running on a Chloe
	pop hl;					// drop return address
	ld bc, $7ffd;			// 128 paging
	xor a;					// LD A, 0
	out (c), a;				// set ROM 0
	rst $18;				// call routine in ROM 0
	defw $2700;				// copy unodos.sys to RAM
	...
	xor a;					// LD A, 0
	out (mmcram), a;		// divMMC page 0
	jp L0242;				// jump to point after test for TR-DOS support
