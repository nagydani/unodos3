...
output_bin "../dos/color", $2000, $00f0;

include "../src/basic.api"
include "../src/zx-uno.api"

org $2000
start:
	ld a, l;				// test for
	or h;					// args
	jr nz, parse;			// jump if found
...
command:
	ld a, (arg1 + 1);		// reject values over 255
	and a;					//
	jp nz, error;			//

	ld a, (arg2 + 1);		// reject values over 255
	and a;					//
	jp nz, error;			//
	
	ld a, (arg2);			// get values
	ld e, a;				// put it in ANY

	ld a, (arg1);			// get register
	cp 65;					// 0-64?
	jp nc, error;			//
